...
- 文本以16进制显示和返回以文本显示
  ...
  ```
  %!xxd 文本以16进制显示  或者%!hexdump -C
  %!xxd -r 文本返回以字符显示  hexdump没有这个功能返回以字符显示。
  ```
...
- c命令会直接进入插入模式进行编辑，例如cw会直接删除当前词然后进入插入模式。
...
- 文本对象命令只能在可视模式或操作符后使用。这些命令或以 "a" 打头，代表选择一个 ("a"n) 包含空白的对象；或以 "i" 带头，代表选择内含 ("i"nner) 对象: 它们不包含空白。另外，空白本身也是内含对象。这样，"内含" 对象总比 "一个" 对象选择较少的文本。
  ...
  - a命令就是在选择的时候带一个分隔符，如果这个文本前后都有分割符，例如一个单词前后都有空格，这时就会选择后面的分隔符，不会前后都选上的，如果这个单词是最后一个单词，只有前面有一个分隔符，这时就会选择前面的空格。总体来说就是a命令会选择一个额外的分隔符。但是只会选择一个，如果有多个的话也是选择一个。
  - i命令是只会选择文本对象本身，是不会选择分隔符的。
...
- 具体配置时，需要写在vimrc中
  ...
  - 这其中第二行的点是字符串拼接的意思，类似于python中的+，拼接字符串
  ...
  - Vim 脚本里，一行的继续是在续行的行首加上反斜杠完成的，所以第二行的行首反斜杠是续行的意思，类似于c里面在一行的末尾写反斜杠，只是这个写在行首。都是续行的意思。反斜杠和后面的点并不是转义的意思，只是一个是续航，一个是拼接字符串。

  - 在匹配end if时不能写成`\<end if\>`，而要写成`\<end\s\+if\>`，空格要用\s表示出来。不能直接写空格，因为其总体来看是正则表达式匹配。
...
<img src="https://pic3.zhimg.com/80/v2-9752311b4c4e51ddb02f59ce25e4830e_720w.webp" style="zoom:80%;" />
...
- **选择和拖拽寄存器（The Selection and Drop Registers）** 有3个寄存器用于保存选中的文本：在Mac和Windows下，**"\***和**"+**，都用于访问系统剪切板；在Linux下，**"+**用于访问系统剪切板，而**"\***包含选中的文本。使用`"+y`命令，可以将当前选中的文文本复制到系统剪切板中。而`"+p`命令，则可以粘贴系统剪切板中的内容。通过剪切板寄存器，能够在不同的Vim编辑器或者是其他应用程序之间，进行文本交换。**"~**包含上次从其它应用程序拖拽到Gvim中的文本。
  - [为什么不能使用剪切板](https://harttle.land/2020/09/04/vim-clipboard.html)
    - 主要原因为当前的vim不支持。
...
- 假设我们有以下代码：
  ...
  - 如下图所示：首先，我们使用A命令进入插入模式并在当前行的末尾增加分号；然后，退回到常规模式；之后，移动到一下行并利用.命令重复插入分号的操作。
    ...
    ![](https://pic4.zhimg.com/80/v2-744e645261febcfc56bfaf14b97adc77_720w)
...
- 假设我们有以下代码：
  ...
  - 在以下实例中：我们希望在每个加号前后分别插入空格，以提高代码的可读性。首先，我们使用f命令查找加号；然后，使用s命令将加号替换为“ + ”；随后，退回到常规模式；之后，就可以利用;.命令查找下一个加号并重复执行替换操作。
    ...
    ![](https://pic3.zhimg.com/80/v2-b24817322b00d67481fba70739bf652a_720w)
...
- 输入`:`命令，使用`/`或`?`搜索命令，都将进入命令行模式。用户可以在屏幕底部的命令行中输入命令，或者使用以下快捷键遍历之前的命令历史，然后点击<Enter>键来执行命令。
  ...
  <img src="https://pic1.zhimg.com/80/v2-eb1966f79ad64badb9b792e68b1519b4_720w" style="zoom:67%;" />
...
- 使用以下命令，将定义ad来代替advertisement：
  ...
  - 当想要输入advertisement时，只要输入ad，然后：
    ...
    - 如果按下Esc键，将插入扩展字符并返回命令模式，Esc键要按两次
    - 如果按下Space或Enter键，那么将在插入扩展字符后，自动增加空格或回车，并停留在插入模式。
  
  - 在插入模式下上面三种都能使用，在命令行模式下(包括查找模式下)只能使用第一种和第三种，第二种不能用
  
    - 命令行模式下第一种和第三种还是在命令行模式(包括查找模式下)，只是第一种直接展开，第二种会加一个空格展开。
  
      ```
      ab Me Message received in<CR>
      ```
  
      - 如果在查找模式下输入Me，在按空格的话，会直接查找，然后在输入一个空格，相当于abbr里面的缩写完全操作完了，会输入一个空格。
...
- GVim（Graphical Vim）和Vim（Vi IMproved）都是Vim编辑器的版本，但它们之间存在一些显著的区别和用途
- GVim有自己的配置文件`~/.gvimrc`和启动程序`gvim`
- VIM
  - Vim是一个高度可配置的文本编辑器，基于更早的编辑器Vi。它主要用于命令行界面（CLI），并且在许多类Unix系统（如Linux、macOS等）以及Windows上都可以使用。
    - **界面**：Vim在终端或命令行界面运行，没有图形界面。
    - **键绑定**：使用键盘快捷键进行所有操作，减少对鼠标的依赖。
    - **资源消耗**：通常占用更少的系统资源，因为它不需要加载图形界面。
    - **可移植性**：由于Vim是命令行程序，它可以在任何支持终端的环境中运行，适用于服务器环境和远程工作。
    - **启动速度**：通常比图形界面的编辑器启动更快。
- GVIM
  - GVim是Vim的图形用户界面版本，提供了Vim的所有功能，并添加了一些图形界面特有的特性。
    - **图形界面**：GVim提供一个图形用户界面，使用户可以使用鼠标进行操作，并且提供菜单和工具栏来访问常用功能。
    - **多窗口支持**：GVim可以更容易地管理多个文件和窗口，提供了更直观的窗口拆分和标签页功能。
    - **字体和颜色**：在GVim中，可以更方便地调整字体和颜色主题，使其在视觉上更具吸引力和可读性。
    - **复制粘贴**：GVim更好地支持系统剪贴板的复制粘贴功能，尤其是在图形环境中。
    - **鼠标支持**：提供对鼠标的全面支持，可以使用鼠标点击、拖拽、选择文本等操作。
    - **多平台**：GVim可以在Windows、macOS和Linux等多个平台上运行，提供一致的图形界面体验。
- 使用Vim的场景：
  - 喜欢在命令行工作，尤其是通过SSH在远程服务器上编辑文件。
  - 需要一个轻量级的编辑器，不需要图形界面的额外功能。
  - 熟悉Vim的快捷键和命令，能够高效地在命令行环境中工作。
- 使用GVim的场景：
  - 喜欢图形界面，并希望通过菜单和工具栏访问功能。
  - 需要更好的字体和颜色支持，尤其是对长期编写和阅读代码时的视觉效果有要求。
  - 喜欢使用鼠标操作，或者需要更好的多窗口和多文件管理。

###### GVim中的色彩测试

- 前景色指的是文本的颜色，即文本本身的色彩。前景色可以翻译为 "foreground color"。
- 背景色指的是文本周围的颜色，即文本所在区域的底色。背景色可以翻译为 "background color"
...
- 关于256色说明
  ...
  - 256色 XTERM（Xterm-256color）是一种支持 256 种颜色的终端仿真器配置。它是 Xterm 终端仿真器的一个变体，通过扩展 ANSI 颜色序列以支持更多的颜色，从而在终端环境中提供更丰富的颜色体验。

  - 通常情况下，传统的终端仿真器仅支持 16 种颜色（8 种基本颜色及其亮色版本），而 256 色 XTERM 支持使用 256 种不同的颜色，其中包括了 8 种灰阶色和 216 种 RGB 颜色。这样，用户可以在终端环境中获得更丰富、更生动的颜色显示效果，使得图形界面和彩色文本在终端中的显示更加丰富多彩。

  - 要使用 256 色 XTERM，首先需要确保你的终端仿真器支持该特性，并且正确配置。然后，你可以在终端中使用支持 256 色的配色方案或设置 Vim 等应用程序使用 256 色 XTERM 配置以享受更加丰富的颜色显示。

- RGB配色是有RGB三个字节的，每一个字节的值从0~255，例如下面RGB格式rgb(128,0,0)，而256色是只有一个字节来表示256种色彩，RGB能表示大概1670w色彩。

  - 例如下面这张256色图，每一个数值表示一种颜色，最多只能有256种颜色，这256种颜色肯定是会有RGB配色的对应颜色的
  ...
  - 256色只有一个字节，每一种颜色用一个字节0~255来表示，例如下面的第二张表中的Xterm Number的值，RGB是三个字节，可以表示1670w颜色，其中表示如下面第二张图中RGB列，也可以用十六进制表示，例如HEX列，用十六进制表示时，一共有6位，每两位是一个字节，正好3个字节。
...
- 我们可以使用:highlight命令，查看当前的语法高亮设定。
  ...
  - 一般情况下xxx代表某些颜色配置，一般情况下显示的都是gui的颜色配置
...
- Vim识别三种不同的终端：**term**，黑白终端；**cterm**，彩色终端；**gui**，Gvim窗口。下面中term cterm gui不加fg bg的时候是设置字体的，不是设置颜色的，字体配置只有下面term里面这几种，其中reverse指的是前景色和背景色反转
...
- 定制状态行
  ...
  - 如果此选项以**%!**开始，它用作表达式。计算此表达式的结果用作选项值。
    ...
    ![](https://pic1.zhimg.com/80/v2-22d6fbe2b2c7c46fa5f1269af9d5c858_720w.webp)
...
- 状态行实例
  ...
  - 实际效果如下图所示：
    ...
    ![img](https://pic2.zhimg.com/80/v2-6a9f9dfa2e4fec129f2f88abfb60e78d_720w.webp)
  ...
  - 需要使用以下命令，自定义高亮显示颜色。
    ...
    ![img](https://pic1.zhimg.com/80/v2-32ed1673ae48d652d1af7d46984d5ab0_720w.webp)
  ...
  - 如下图所示，状态行的文件名部分，在插入状态时显示为橘色背景，这样就能很明显地提醒我们所处的状态：
    ...
    ![img](https://pic3.zhimg.com/80/v2-b732e40d4063a77a2ea15953ae1bb97a_720w.webp)
...
- 预览窗口和quickfix不是一个东西，但是使用预览窗口vim必须包含quickfix特性。
- 这个预览窗口和上面的标签列表最大的不同是这个预览窗口可以替换，意思为多次打开也就是一个，总是在这一个预览窗口中显示最新的tag，但是标签列表会直接打开多个buffer或者多个窗口，需要一一关闭。

  - 预览窗口看的tag最后也会在buffer中体现，例如打开之后虽然总是在一个预览窗口中体现，但是使用快捷键bb看到的buffer也会是多个，相当于打开了文件。
...
- 插件对(), [], {}, <>, ‘’, “”，标记语言xml html 有效，其中xml中前后的标记用t代表
  ...
  - 对上面说的这几种括号除了引号，因为引号左右都是相同的，在添加命令ys和修改命令cs下，都是左括号时添加在包围的字符前后都加一个空格，右括号是不加空格的。

    ```
    例如对于字符abc来说，ysiw(命令最后的效果为（ abc ）,ysiw)的效果为(abc)
    ```
...
- 块可视模式下只能在每行的结尾添加一些东西，这时在结尾添加的东西不是固定的，是紧跟在每行的结尾的。但是在中间的某些不规则区域后面统一添加一些东西，例如在不规则的等号后面添加东西，块可视模式就不行了，在中间添加的话，添加到规则区域的后面，意思是只能添加到最大框的后面。这时只能选择上面那种工作模式。
  - 行可视模式下是不能统一在行首或者行尾添加东西的，只能在块可视模式下，而且块可视模式下只能在规则的方框后面或者前面添加，如果不规则的话只能在最大框的后面添加，其他的行也是跟最大的框添加的位置一样。如果想在不规则的行后面统一添加东西，只能使用上述插件。
  - 块可是模式下只能使用大写的I和A，不能使用小写的，因为是整个区域，我们只能在前面或者后面写入，不能在中间写入。
...
- 例如
  ...
  - 就可以借助 tabular 快速简单的实现，在 vim 中 `shift+v` 进入视图模式选中这两行，然后在命令行模式输入 `Tabularize / /l0` 就可以了。将这个命令拆分一下：
    - `/` 表示以  空格符作为分割符
    - `/l0` 表示左对齐，每一个拆分后的 filed 左边补充 **0** 个空格，即第一行被分割符号拆分后是：#define, ,TEST0, ,1000；第二行被分割符号拆分后是：#define, ,TEST0_LONG_FORMAT, ,2000
  ...
  - 如果想让实现下面的效果该如何呢
    ...
    - 上述效果可以理解为，以  空格符为分割符，左边左对齐，中间右对齐，右边左对齐，拆分的 filed 之间不会补充额外的  符号。知道了这样的目的，命令就是  `Tabularize / /l0c0r0c0l0` ，其中 `c0` 指定的是两个  分割符的对齐格式，简单一点可以直接  `Tabularize / /r0` 就可以了。因为被  拆分的 filed，只有 `TEST0` 和 `LONG_FORMAT_TEST0` 需要右对齐，其他的 filed 不管是左对齐、居中对齐还是右对齐，效果都是一样的，加上 **tabular 具有循环执行设置的对齐方法的功能**，所以让所有的 filed 右对齐就可以了。

- 如果有更多的 filed，那么回按照对齐的命令循环处理，这次我就用一个 `,` 符号作为分割符的例子来展示：
  ...
  - 如果使用 `,` 作为分割符，并且对齐分别是左对齐、右对齐、左对齐、右对齐，正常需要执行的完整命令是  `Tabularize /,/l0c0r0c0l0c0r0` ，但是 tabular 有自动循环执行之前对齐命令的功能，可以简写为   `Tabularize /,/l0c0r0c0` 就可以了，后面的`l0c0r0`就可以省略了，对齐后的效果是
...
- 通过查看fzf的Readme.md，查看源码如下
  ...
  * [License](#license)
  ...
  - 小括号里面写的标题名不用管大小写以及空格和-可以互相替代，例如
  
    ```
    [a](#aaa)   或者[a](#AAA) 或者[a](#aAa) 这三种都可以跳到aaa标题下面，不用管大小写 
    
    aaa
    
    bbb
    
    [a](#aaa-bbb)  标题中的空格可以用-代替，也可以用原来的空格，其他的分隔符不可以，只能是-代替空格，或者反过来标题中的-用TOC中的空格代替，即两者可以互相代替，其他的分隔符不可以互相代替
...
- 在这期间，一旦我们的远程连接工具所在的工作电脑出现断网或断电的情况，那么很多耗时较长的操作就会因此中断，这是所有运维、开发同学都很头疼的一个问题。下面，我们就先给大家列出一些典型的痛点。
  - 痛点一：大数据传输
    - 相信做过 Linux 服务运维的同学，都用 scp 进行过服务器间的大文件网络传输。一般这需要很长的时间，这期间，如果工作电脑出现断网或者断电的情况，则会导致远程连接工具无法与服务器通信，从而使得它所控制的数据传输也因此中断。
      - scp传输是在两个服务器中，并不是从工作电脑传输到服务器中。我们的工作电脑断网，会导致我们的远程工具和服务器通信断开，然后服务器里面的scp命令也会中断。
      - 主要是我们工作的电脑和服务器连接中断，会导致服务器上对应的终端执行的命令也中断，服务器上的命令不会自己继续执行的。后面的痛点二也是这个原理，我们在本地连接服务器在服务器山执行编译命令，如果本地断网，对应的服务器上编译的命令也会断掉。
...
- 很多以set设置的选项都有格式的设置以及打印的东西是什么的设置。
